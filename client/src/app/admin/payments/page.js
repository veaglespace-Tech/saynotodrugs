  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Payments & Donations</h1>
          <p className="text-slate-400 mt-1">Track all contributions made towards the campaign.</p>
        </div>
        ...
        <div className="p-6 border-b border-white/5 bg-white/5">
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="block w-full pl-11 pr-4 py-3 border border-white/10 bg-white/5 rounded-xl text-white placeholder-slate-500 focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all"
            />
          </div>
        </div>
              ...
              {filteredDonations.map((donation) => (
                <tr key={donation.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-bold text-white">{donation.user?.name}</div>
                    <div className="text-sm text-slate-400">{donation.user?.email}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-lg font-bold text-emerald-500">₹{donation.amount}</div>
                  </td>
                  <td className="px-6 py-4">
                    <code className="text-xs bg-white/5 text-slate-300 px-2 py-1 rounded-md border border-white/10">
